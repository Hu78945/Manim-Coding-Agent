...
2. That input is dropped into a system prompt that casts the model as an
   expert Manim developer (Python, `manim` library) and sent to Gemini via
   `@google/genai`.
...
- `index.js` — CLI entry point: loads `.env`, prompts for a topic, creates a
  workspace folder, runs the agent loop, prints progress + final summary.
- `src/agent.js` — the agent loop (`runAgent`): uses `ai.chats.create` with
  the system prompt + tool declarations, dispatches `response.functionCalls`
  via `callTool`, feeds results back with `createPartFromFunctionResponse`,
  repeats until the model stops calling tools or `MAX_TURNS` (25) is hit.
- `src/systemPrompt.js` — builds the "expert Manim developer" system prompt
  (workflow instructions, Manim docs link, tool usage conventions).
...
- `src/tools/schema.js` — the `FunctionDeclaration[]` for all five tools
  (`write_file`, `read_file`, `delete_file`, `list_files`, `execute_python`)
  plus `callTool`, the dispatch table the agent loop calls into.
...
- Copy `.env.example` to `.env` and set `GEMINI_API_KEY` (and optionally
  `GEMINI_MODEL`, defaults to `gemini-3-pro-preview`). Loaded via `dotenv`.
